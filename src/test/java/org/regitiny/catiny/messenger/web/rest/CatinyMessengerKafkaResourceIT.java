...
import org.regitiny.catiny.messenger.config.KafkaProperties;
...
class CatinyMessengerKafkaResourceIT {
    ...
    void setup() {
        ...
        CatinyMessengerKafkaResource kafkaResource = new CatinyMessengerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/catiny-messenger-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/catiny-messenger-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
